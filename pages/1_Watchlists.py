...
# Bug 4 fix: get_fresh_creds() handles token refresh transparently.
try:
...
def get_sheet_data(spreadsheet_id, range_name):
    """Fetch data using the official Sheets API."""
    try:
...
def update_sheet_full(spreadsheet_id, range_name, df):
    """Overwrite the sheet entirely (required for deletions)."""
...
# ── 4. WATCHLIST RENDERING ────────────────────────────────────────────────────
def analyze_and_render_watchlists(watchlist_df, spreadsheet_id):
    watchlist_df['Price Added'] = pd.to_numeric(
        watchlist_df['Price Added'], errors='coerce'
    ).fillna(0)
    ...
    for tab, current_list in zip(tabs, list_names):
        with tab:
            ...
            tickers = [
                t if ('.NS' in t or '.BO' in t) else f"{t}.NS"
                for t in df['Instrument']
            ]
            ...
            if results:
                ...
                merged_df = pd.merge(
                    df, analysis_df,
                    left_on='Instrument', right_on='Ticker',
                    how='left',
                )

                merged_df['Qty']              = merged_df['Price Added'].apply(
                    lambda x: int(round(100000 / x)) if x > 0 else 0
                )
                ...
                merged_df['Chart'] = (
                    ...
                    + merged_df['Instrument'].str.replace('.NS', '').str.replace('.BO', '')
                    + "/"
                )
                ...
                if edited_df['🗑️ Delete'].any():
                    if st.button("🗑️ Confirm Deletion", key=f"btn_{current_list}", type="primary"):
                        ...
                        mask = ~(
                            (watchlist_df['List Name'] == current_list)
                            & (~watchlist_df['Instrument'].isin(to_keep))
                        )
...
# Bug 3 fix: display email from session_state, never the raw token.
                ...
                with st.spinner("Fetching historical price..."):
                    try:
                        ...
                        h  = tk.history(
                            ...
                            end=(date + datetime.timedelta(days=7)).strftime("%Y-%m-%d"),
                        )
                        if not h.empty:
                            row = [[
                                final_cat,
                                ticker.replace('.NS', '').replace('.BO', '').upper(),
                                h.index[0].strftime("%Y-%m-%d"),
                            ...
                            st.warning("No price data found for that date. Try a trading day.")
                    except Exception as e:
                        st.error(f"Error: {e}")
